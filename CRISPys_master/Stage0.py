...
from CRISPys_master import globals
from CRISPys_master.make_tree_display_CSV import tree_display
from CRISPys_master.CasSites import fill_genes_targets_dict
from CRISPys_master.Stage1 import default_alg, gene_homology_alg
from CRISPys_master.Distance_matrix_and_UPGMA import MITScore, ccTop, gold_off_func, ucrispr, default_on_target
from CRISPys_master.Metric import cfd_funct
from CRISPys_master.SubgroupRes import SubgroupRes
from CRISPys_master.Candidate import Candidate
from CRISPys_master.CRISPR_Net.CrisprNetLoad import load_crispr_net
from CRISPys_master.MOFF.MoffLoad import load_moff
from CRISPys_master.DeepHF.LoadDeepHF import load_deephf
from CRISPys_master.singletons import singletons_main
...
def choose_scoring_function(input_scoring_function: str, family_path: str):
    ...
    if input_scoring_function == "CrisprMIT":
        ...
        logging.error("Did not specify valid scoring function")
...
def CRISPys_main(fasta_file: str, output_path: str, output_name: str = "crispys_output",
    ...
    :param run4chips: if the output is intended to use with chips don't filter sgRNAs that are not in list of interest genes
    ...
    if genes_of_interest_file != "None":
        ...
        if not genes_of_interest_set:
            logging.info(f"Job ended  for {output_name}. This family contains no genes from the list of interest")
            return []
    else:
    ...
    if len(genes_exons_dict) == 1:
        logging.debug("family contains a single gene")
        if not singletons:
            return []
    ...
    logging.info(f"Job ended successfully for {output_name}. Output files created")
    return res
...
def parse_arguments(parser_obj: argparse.ArgumentParser):
    ...
    parser_obj.add_argument('--min_desired_genes_fraction', '-min_genes_frac', type=float, default=-1.0,
                            help="When using the gene_homology algorithm and specifying a list of gene of interest."
                                 ...
                                 "internal nodes of size n that has n/2 genes of interest or less."
                                 "If set to 0.0, sgRNAs will only be designed for internal nodes that have at least "
                                 "one gene of interest. "
                                 "If set to -1.0, sgRNAs will be designed for all internal nodes.")
