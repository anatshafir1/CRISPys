...
def get_sites(gene, min_length=20, max_length=20, start_with_G=False, where_in_gene = 1, PAMs= ["GG"]):
    ...
    # loop over different PAM's
		for i in range(len(PAMs)):
		  ...
		  # functions that take targets of length 23 (used for scoring scheme the need the PAM, e.g gold_off)
	...
	for i in range(len(exons_lst)):
		...
		if i == 0:
			if range_in_gene[0] < lengths[i]:
				...
				list_of_targets += get_sites(exons_lst[i][range_in_gene[0] : min(lengths[i], range_in_gene[1])], df, min_length, max_length, start_with_G, where_in_gene = 1, PAMs)
		elif max(range_in_gene[0], lengths[i-1]) < min(lengths[i], range_in_gene[1]):
			list_of_targets += get_sites(exons_lst[i][max(range_in_gene[0]  - lengths[i-1], 0) : min(lengths[i] - lengths[i-1], range_in_gene[1] - lengths[i-1])], df, min_length, max_length, start_with_G, where_in_gene = 1, PAMs)
	return list_of_targets
