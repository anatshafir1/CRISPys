...
def find_set_cover(best_permutations_DS, sg_genes_dict, thr, genes_sg_dict=None):
    '''for now, might won't work in a case when there is a gene that isn't covered by any of the permutations in the best_permutations. not finished. can make it more readble'''
    ...
    if genes_sg_dict:
        for gene, targets in genes_sg_dict.items():
            ...
            c.fill_default_fields(sg_genes_dict[targets[0]])
        ...
        while i < (len(temp_best_perm_DS)):
            ...
            if len(new_genes_coverd) == 0:
                ...
                continue
                    ...
                    new_genes_coverd) >= best_num_of_coverd:
