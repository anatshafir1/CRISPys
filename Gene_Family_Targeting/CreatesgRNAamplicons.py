...
def create_single_sgrna_amplicons(sorted_sgrnas: List[Candidate], genes_exons_seq_dict: Dict[
    ...
    for sgrna in sorted_sgrnas:
        ...
        for gene_name in sgrna_targets_dict:
            ...
            gene_orig_exons_dict = genes_exons_seq_dict[gene_name][1]
            for exons_targets_dict in exons_targets_dicts_list:
                gene_candidate_amplicons = construct_amplicons(genes_snps_dict[gene_name],
                                                               exons_targets_dict,
                                                               max_amplicon_len, primer_length, distinct_alleles_num,
                                                               target_surrounding_region, min_amplicon_len, 0, target_len,
                                                               0)
                if gene_candidate_amplicons[0]:
                    filt_sorted_gene_candidate_amplicons = filter_redundancies_and_sort(gene_candidate_amplicons[0], 0)
                    gene_amplicons = get_gene_family_primers(genes_exons_seq_dict[gene_name][0],
                                                         filt_sorted_gene_candidate_amplicons,
                                                         out_path,
                                                         primer3_core_path, n,
                                                         ...
                                                         max_amplicon_len,
                                                         genes_snps_dict[gene_name], 0,
                                                         min_amplicon_len,
                                                         0, family_targeting)
                    if gene_amplicons:
                        gene_amplicons_with_primers[gene_name] = gene_amplicons
                        amps_for_gene = True
                        break
                    else:
                        continue
...
def get_single_targets(gene_targets_dict: Dict[int, List[Family_Target_Obj]]) -> Tuple[Family_Target_Obj, Family_Target_Obj]:
    ...
    for exon in gene_targets_dict:
        if gene_targets_dict[exon]:
            targets_list.append(gene_targets_dict[exon][0])
    up_target, down_target = targets_list[0], targets_list[1]
    ...
    for index, sgrna_pair in enumerate(sorted_sgrna_pairs):
        ...
        for gene_name in sgrna_pair_targets_dict:
            ...
            for exons_targets_dict in exons_targets_dicts_list:
                ...
                else:  # Current sgRNA pair is on different exons. Construct Single Target amplicons
