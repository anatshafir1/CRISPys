...
def return_upgma(seq_list, names_list, df, cfd_dict = None):
	...
	if df == Metric.cfd_funct:
		...
		metric_seq_list = list(map(lambda t: Metric.pos_in_metric_cfd_np(t, cfd_dict), seq_list)) #to uncomment

	#	df = Metric.find_dist_t  #if prev line is not  is use #to uncomment
		df_metric = Metric.find_dist_np
	matrix = UPGMA.make_initiale_matrix(df_metric,metric_seq_list)
