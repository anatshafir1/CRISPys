...
import pandas as pd
from MultiplexToSingleplex import construct_singleplex_candidates
from Amplicon_Obj import Amplicon_Obj
from Target_Obj import MultiplexTarget
...
def valid_amplicon(i: int, j: int, exon_snps_list: List[SNP_Obj], distinct_alleles_num: int) -> bool:
    """
    Check if SNPs i to j in exon_snps_list are enough to distinguish between the different alleles. .
    ...
    for m in range(i, j + 1):
        ...
        if k > 0:  # Tool 2 in use.
            ...
            if curr_target.start_idx <= exon_snps_list[m].position <= curr_target.end_idx:  # SNP on target
                if any(allele_set.intersection(curr_target.cut_alleles) == len(curr_target.cut_alleles) for allele_set
                       in exon_snps_list[m].alleles_sets_lst):  # SNP match expected alleles to be cut
                    valid_snps_list.append(exon_snps_list[m])
...
def calculate_snps_statistics(valid_snps_for_target: List[SNP_Obj]) -> Tuple[float, float]:
    """
    ...
    counts_dict = {}
    k = 0
    j = len(valid_snps_for_target) - 1
    used_sets = {snp.position: [] for snp in valid_snps_for_target}
    while k < j + 1:
        ...
        for set1 in curr_snp.alleles_sets_lst:
            if len(set1) == 1:
                ...
                set1_done = False
                for m in range(k + 1, j + 1):
                    next_snp = valid_snps_for_target[m]
                    for set2 in next_snp.alleles_sets_lst:
                        if set2 in used_sets[next_snp.position]:
                            continue
                        if len(set2) == 1:
                            continue
                        intersect = set1.intersection(set2)
                        if len(intersect) == 1:  # intersection is singleton
                            set1_done = True
                            if str(intersect) not in counts_dict:
                                counts_dict[str(intersect)] = 1
                            else:
                                counts_dict[str(intersect)] += 1
                            used_sets[next_snp.position].append(set2)
                            break
                    if set1_done:
                        break
        k += 1
...
def create_candidate_amplicon(snps_median: float, snps_mean: float, candidate_amplicon_snps_lst: List[SNP_Obj],
                              ...
                              target_surrounding_region: int, min_amplicon_len: int, target_len: int,
                              multiplex: int) -> Amplicon_Obj:
    """
    ...
    if multiplex:
        min_amp_start_index = target.down_start - max_range_from_target if target.down_end > last_snp.position else last_snp.position - max_range_from_snp
        max_amp_end_index = target.up_end + max_range_from_target if target.up_end < first_snp.position else first_snp.position + max_range_from_snp
    else:
...
def valid_target_distance(target: MultiplexTarget, max_amplicon_len: int, primer_length: int,
                          target_surrounding_region: int) -> bool:
    """
    validate that upstream target and downstream target are not too far apart for an amplicon of size "max_amplicon_len".

    :param target: multiplex target with upstream target and downstream target.
    :param max_amplicon_len: maximum length of the amplicon.
    :param primer_length: minimum length of the primer sequence.
    :param target_surrounding_region: buffer regions around sgRNA target (upstream and downstream) where primers are not allowed.
    :return: True if targets are not too far apart, False otherwise.
    """
    max_dist = max_amplicon_len - 2 * primer_length - 2 * target_surrounding_region
    if target.down_end - target.up_start > max_dist:
        return False
    else:
        return True


def valid_sgrna_multiplex_target(i: int, j: int, exon_snps_list: List[SNP_Obj], distinct_alleles_num: int,
                                 curr_target: MultiplexTarget, max_amplicon_len: int, primer_length: int,
                                 target_surrounding_region: int) -> List[SNP_Obj]:
    """
    Check if current Multiplex target can be used to construct an amplicon.
    ...
    last_snp = exon_snps_list[j]
    # Make sure that target is not too far upstream or downstream from first and last SNPs
    if not (curr_target.down_end < first_snp.position + max_range_from_snp and curr_target.up_start >
            last_snp.position - max_range_from_snp):
        return []
    # Create a list of SNPs that can be used to calculate the Amplicon statistics
    for m in range(i, j + 1):
        # Add to valid_snps_list SNPs that do not fall on the target or the target buffer region - for all Tools
        if (not (curr_target.up_start - target_surrounding_region <= exon_snps_list[m].position <=
                 curr_target.up_end + target_surrounding_region)
                and not (curr_target.down_start - target_surrounding_region <= exon_snps_list[m].position <=
                         curr_target.down_end + target_surrounding_region)):
            valid_snps_list.append(exon_snps_list[m])
    if len(valid_snps_list) == 0:  # if no SNPs can be used do discern between the alleles
        return []
    # Make sure that the new SNPs list in enough for a valid amplicon (enough to distinguish between all the alleles)
...
def get_candidate_amplicons(i: int, j: int, exon_snps_lst: List[SNP_Obj], distinct_alleles_num: int,
    ...
    :param exon_num: current exon number, for which amplicons are constructed.
    :param target_surrounding_region: buffer regions around sgRNA target (upstream and downstream) where primers are not allowed.
    :param min_amplicon_len: minimum length of the amplicon.
    :param k: number of alleles to target.
    :param target_len: number of nucleotides in sgRNA target: PAM + protospacer.
    :param multiplex: choose whether to plan 2 sgRNA or 1 sgRNA.
    ...
    for target in exon_targets_lst:
        ...
        if len(valid_snps_for_target) > 0:
            ...
            snps_median, snps_mean = calculate_snps_statistics(
                valid_snps_for_target)  # statistics are counted for SNPs that are not located near the target (target surrounding region)
            if snps_median > 0:
...
def save_results_to_csv(res_amplicons_lst: Tuple[List[Amplicon_Obj], List[Amplicon_Obj]], out_path: str, k: int, multiplex: int):
    ...
    """
    dicts_list = []
    singleplex_dicts_list = []
    if multiplex:
        for amplicon in res_amplicons_lst[0]:
            dicts_list.extend(
                [amplicon.scaffold_amplicons[scaffold_amplicon].to_dict(amplicon.target.rank, k, multiplex) for scaffold_amplicon in
                 amplicon.scaffold_amplicons])
        df = pd.DataFrame(dicts_list)
        df.to_csv(out_path + "/multiplex_results.csv", index=False)

        for amplicon in res_amplicons_lst[1]:
            singleplex_dicts_list.extend(
                [amplicon.scaffold_amplicons[scaffold_amplicon].to_dict(amplicon.target.rank, 0, 0) for scaffold_amplicon in
                 amplicon.scaffold_amplicons])
        df = pd.DataFrame(singleplex_dicts_list)
        df.to_csv(out_path + "/singleplex_results.csv", index=False)
    else:
        for rank, amplicon in enumerate(res_amplicons_lst[0]):
            dicts_list.extend(
                [amplicon.scaffold_amplicons[scaffold_amplicon].to_dict(rank + 1, k, multiplex) for scaffold_amplicon in
                 amplicon.scaffold_amplicons])
        df = pd.DataFrame(dicts_list)
...
def filter_redundancies_and_sort(candidate_amplicons_list: List[Amplicon_Obj],
                                 singleplex_candidate_amplicons_list: List[Amplicon_Obj] = None, k: int = 1,
                                 multiplex: int = 1) -> List[Amplicon_Obj]:
    """
    sort the amplicon candidates according to the used algorithm method. rank them and add a rank attribute to the objects

    :param candidate_amplicons_list: List of candidate amplicons to be sorted and ranked
    :param singleplex_candidate_amplicons_list:
    :param k: number of alleles to target with a single gRNA.
    :param multiplex: choose whether to plan 2 sgRNA or 1 sgRNA.
    :return: list of sorted and ranked amplicons
    """
    if k > 0:
        sorted_amplicon_obj = sorted(candidate_amplicons_list,
                                     key=lambda amplicon: (
                                         amplicon.target.chosen_sg_score, amplicon.snps_median,
                                         amplicon.snps_mean, len(amplicon.snps)), reverse=True)
    elif multiplex:
        combined_list = candidate_amplicons_list + singleplex_candidate_amplicons_list
        sorted_amplicon_obj = sorted(combined_list,
                                     key=lambda amplicon: (
                                         -amplicon.target.rank, amplicon.snps_median,
                                         amplicon.snps_mean, len(amplicon.snps)), reverse=True)
    else:
        sorted_amplicon_obj = sorted(candidate_amplicons_list,
...
def get_amplicons(max_amplicon_len_category: int, primer_length: int, target_surrounding_region: int, cut_location: int,
                  ...
                  pams: Tuple[str], target_len: int, primer3_core_path: str, n: int, filter_off_targets: int, k: int,
                  multiplex: int) -> Tuple[List[Amplicon_Obj], List[Amplicon_Obj]]:
    """

    :param max_amplicon_len_category: category of maximum length of the amplicon.
    :param primer_length: minimum length of the primer sequence.
    :param target_surrounding_region: buffer regions around sgRNA target (upstream and downstream) where primers are not allowed.
    ...
    if k > 0 or multiplex:  # Tool 2 in use, targeting k alleles. SNPs allowed in target sequences.
    ...
    sorted_targets_for_singleplex_lst = sorted(targets_for_singleplex_lst, key=lambda trg: trg.rank)
    if multiplex:
        singleplex_candidate_amplicons_list = construct_singleplex_candidates(sorted_targets_for_singleplex_lst, gene_snps_dict, max_amplicon_len, primer_length,
                                                  distinct_alleles_num, target_surrounding_region, min_amplicon_len, 0,
                                                  target_len, 0)
        filt_sorted_candidate_amplicons_list = filter_redundancies_and_sort(candidate_amplicons_list, singleplex_candidate_amplicons_list, k, multiplex)
    else:
        filt_sorted_candidate_amplicons_list = filter_redundancies_and_sort(candidate_amplicons_list, [], k, multiplex)
    ...
    if filter_off_targets:  # find gRNA off-targets and filter amplicons by scores, then get primers.
        ...
        if len(off_trg_filt_sorted_candidate_amplicons) == 0:
            ...
            sys.exit()
        amplicon_obj_with_primers = get_primers(gene_exon_regions_seqs_dict, off_trg_filt_sorted_candidate_amplicons,
                                                out_path, primer3_core_path, n,
                                                ...
                                                multiplex, primer_length, min_amplicon_len, target_len, genome_fasta_file
                                                )
    ...
    if len(amplicon_obj_with_primers[0]) > 0 or len(amplicon_obj_with_primers[1]) > 0:
        save_results_to_csv(amplicon_obj_with_primers, out_path, k, multiplex)
        ...
        if max_amplicon_len_category < 3:
            get_amplicons(max_amplicon_len_category + 1, primer_length, target_surrounding_region, cut_location,
                          ...
                          primer3_core_path, n, filter_off_targets, k, multiplex)
        else:
