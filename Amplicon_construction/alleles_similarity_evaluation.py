...
def get_genomic_sites(out_path: str, fasta_file: str, filtered_allele_df) -> List[str]:
    """
...
def get_overlapping_exons(exon_indices_dict: Dict[str, List[Tuple[int, int]]],
                          alleles_exons_aligned_dict: Dict[str, str],
                          alignment_len: int) -> Tuple[Dict[str, List[Tuple[int, int]]], int]:
    aligned_overlapping_exons_dict = {allele: [] for allele in exon_indices_dict}
    prev_allele_idx = 0
    curr_allele_idx = 0
    alleles_to_loop_over_again = dict()
    nuc_count_dict = dict()
    for allele in exon_indices_dict:
        nuc_count_dict[allele] = 0
    exon_num_dict = dict()
    for allele in exon_indices_dict:
        exon_num_dict[allele] = 0
    num_of_exons = 0

    while curr_allele_idx < alignment_len:

        for allele in alleles_exons_aligned_dict:
            curr_allele = alleles_exons_aligned_dict[allele]
            curr_exon = exon_num_dict[allele]
            if curr_allele[curr_allele_idx] != "-":
                curr_exon_len = abs(exon_indices_dict[allele][curr_exon][1] - exon_indices_dict[allele][curr_exon][0]) + 1
                while nuc_count_dict[allele] < curr_exon_len:
                    if curr_allele[curr_allele_idx] != "-":
                        nuc_count_dict[allele] += 1
                    curr_allele_idx += 1
                for al in alleles_exons_aligned_dict:
                    if al != allele and al not in alleles_to_loop_over_again:
                        alleles_to_loop_over_again[al] = alleles_exons_aligned_dict[al]
                if curr_allele_idx <= alignment_len:
                    exon_num_dict[allele] += 1
                    nuc_count_dict[allele] = 0
                break
            else:
                alleles_to_loop_over_again[allele] = curr_allele

        legit_exon_dict = {allele: False for allele in alleles_to_loop_over_again}
        for allele in alleles_to_loop_over_again:
            curr_allele = alleles_exons_aligned_dict[allele]
            curr_exon = exon_num_dict[allele]
            curr_exon_len = abs(exon_indices_dict[allele][curr_exon][1] - exon_indices_dict[allele][curr_exon][0]) + 1
            indel_cnt = curr_allele[prev_allele_idx:curr_allele_idx].count("-")
            nuc_cnt = curr_allele_idx - prev_allele_idx - indel_cnt
            if nuc_cnt > 45:
                legit_exon_dict[allele] = True

            nuc_count_dict[allele] += nuc_cnt
            if nuc_count_dict[allele] >= curr_exon_len and curr_allele_idx <= alignment_len:
                exon_num_dict[allele] += 1
                nuc_count_dict[allele] = nuc_count_dict[allele] - curr_exon_len
        if all(legit_exon_dict[allele] is True for allele in legit_exon_dict):
...
def get_legit_exons_regions(annotations_file_path: str, out_path: str, genome_fasta_file: str) -> \
    ...
    aligned_overlapping_exons_dict, num_of_exons = get_overlapping_exons(exon_indices_dict, alleles_exons_aligned_dict, alignment_len)
    return aligned_overlapping_exons_dict, num_of_exons, allele_strand_dict


def get_exon_dict(aligned_overlapping_exons_dict, exon_num, exon_surrounding_seq_len, allele_strand_dict):
