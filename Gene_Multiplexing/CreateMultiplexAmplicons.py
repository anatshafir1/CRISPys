...
def create_multiplex_amplicons(multiplex_targets_list: List[MultiplexTarget],
                               ...
                               min_amplicon_len: int, target_len: int, out_path: str, primer3_core_path: str, n: int,
                               amplicon_ranges: List[Tuple[int, int]],
                               max_amplicon_len_category: int) -> Tuple[Dict[str, List[Amplicon_Obj]], Dict[str, MultiplexTarget]]:
    multiplex_amplicons_dict = {}
    multiplex_seq_to_object = {}
    failed_targets = {}
    for index, multiplex_target in enumerate(multiplex_targets_list):
        rank = index + 1
        multiplex_target.rank = rank
        ...
        multiplex_candidate_amplicons = construct_amplicons(gene_snps_dict, sgrna_pair_targets_dict, max_amplicon_len,
            primer_length, distinct_alleles_num, target_surrounding_region, min_amplicon_len, 0, target_len, 1)
        if not multiplex_candidate_amplicons:  # Failed to construct Multiplex Candidate amplicons. Try singles
            ...
            singleplex_amplicons = get_singleplex_amplicons(
                ...
                amplicon_ranges[max_amplicon_len_category - 1], original_exon_indices_dict, failed_targets, 0)
            if not singleplex_amplicons:  # Failed to construct Single Target amplicons. sgRNA pair failed.
                ...
                multiplex_amplicons_dict[multiplex_target_str] = singleplex_amplicons
                multiplex_seq_to_object[multiplex_target_str] = multiplex_target
        else:  # Constructed multiplex candidate amplicons. Find primers
            filt_sorted_multiplex_candidate_amplicons = filter_redundancies_and_sort(multiplex_candidate_amplicons, 0)
            multiplex_amplicons = get_gene_family_primers(
                gene_exon_seqs_dict, filt_sorted_multiplex_candidate_amplicons, out_path,
                primer3_core_path, n, amplicon_ranges[max_amplicon_len_category - 1], distinct_alleles_num,
                target_surrounding_region, original_exon_indices_dict, max_amplicon_len, gene_snps_dict,
                0, min_amplicon_len, 1, 0)
            ...
            if not multiplex_amplicons:  # Failed to find primers for multiplex candidate amplicons. Try singles
                ...
                multiplex_amplicons_dict[multiplex_target_str] = multiplex_amplicons
                multiplex_seq_to_object[multiplex_target_str] = multiplex_target

    return multiplex_amplicons_dict, multiplex_seq_to_object
