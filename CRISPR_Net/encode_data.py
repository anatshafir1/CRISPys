...
def load_Listgarten_indel_dataset():
    print("Loading dataset I/2...")
    ele_indel_df = pd.read_csv("../data/Dataset I (indel&mismatch)/dataset I-2/elevation_6gRNA_wholeDataset.csv")
    print(len(ele_indel_df), len(ele_indel_df[ele_indel_df['label']>0]))
    ele_indel_code = []
    ele_indel_reads = []
    for idx, row in ele_indel_df.iterrows():
        on_seq = row['crRNA']
        off_seq = row['DNA']
        read_val = row['read']
        en = Encoder_sgRNA_off.Encoder(on_seq=on_seq, off_seq=off_seq, with_reg_val=True, value=read_val)
        ele_indel_code.append(en.on_off_code)
        ele_indel_reads.append(en.value)

    ele_indel_code = np.array(ele_indel_code)
    ele_indel_reads = np.array(ele_indel_reads)
    ele_indel_label = np.zeros(len(ele_indel_reads))
    ele_indel_label[ele_indel_reads > 0] = 1
    # print("Dataset size:", ele_indel_code.shape, "positive num:", len(ele_indel_label[ele_indel_label > 0]))
    return ele_indel_code, ele_indel_label
